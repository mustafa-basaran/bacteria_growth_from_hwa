...
//	double Iz = MR2;   // Mya's version
	...
	new_cell.AngularVelocity = scale(Txy, 1.0 / (gama_rotational));
	...
	// angular velocity
	DoubleCoord vap = rotate(rcmp, new_cell.AngularVelocity.z*dt);
	DoubleCoord vaq = rotate(rcmq, new_cell.AngularVelocity.z*dt);

	// total velocity

	DoubleCoord vcm = scale(new_cell.Velocity, dt);

	// Integrate to find new positions
	new_cell.Position.p = sum(cm,sum(vcm, vap));
	new_cell.Position.q = sum(cm, sum(vcm, vaq));;
	//L_cur += (new_cell.Position.p.x - new_cell.Position.q.x) * (new_cell.Position.p.x - new_cell.Position.q.x);
	//L_cur += (new_cell.Position.p.y - new_cell.Position.q.y) * (new_cell.Position.p.y - new_cell.Position.q.y);
	//L_cur = sqrt(L_cur);
	//new_cell.Length = L_cur;
}
