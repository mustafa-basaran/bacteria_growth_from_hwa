	...
	for (int icolony = 0; icolony < Ini.ColonyNumber; icolony++)
	{
		while (icell<Ini.ColonySize)//changed by YueYan
		{
			...
			while (CheckOverlap==true)
			{
				...
				thetaPos = 2.0*PI*((float)(icell-1)/Ini.ColonySize);
...
int ReadFirstCells(Cell* cells, double L_divide, double radius, UniformGrid& Grid, Inputs& Ini)
{
	...
	int RandType;
	int asdg;
	std::ifstream file("C:/Users/mind/Desktop/code_init_radial/long_involved.txt");
	...
	current_cell_no = current_cell_no;
		...
		{	
			...
			//{
				...
				cells[addedcell].Radius = radius;
				radiusPos = (float)Ini.ColonyRadius;
				if (new_cell_array[icell * 28 + 5] < 0.7 && new_cell_array[icell * 28 + 8] < 0.7 && new_cell_array[icell * 28 + 3]<150.0 && new_cell_array[icell * 28 + 4] <150.0 && new_cell_array[icell * 28 + 6] < 150.0 && new_cell_array[icell * 28 + 7] < 150.0 && new_cell_array[icell * 28 + 3] >  -150.0 && new_cell_array[icell * 28 + 4] > -150.0 && new_cell_array[icell * 28 + 6] > -150.0 && new_cell_array[icell * 28 + 7] > -150.0) {
					...
					cells[addedcell].Length = sqrt(L);
					//cells[addedcell].Length = new_cell_array[icell * 28 + 9];

					v = DoubleCoord(0, 0, 0);
					va = DoubleCoord(0, 0, 0);
					cells[addedcell].Position.p = p;
					cells[addedcell].Position.q = q;
					cells[addedcell].Position.time_p = 0;
					cells[addedcell].Position.time_q = 0;
					cells[addedcell].Position.age_p = 0;
					cells[addedcell].Position.age_q = 0;
					//v.x = new_cell_array[icell * 28 + 13];
					//v.y = new_cell_array[icell * 28 + 14];
					//v.z = new_cell_array[icell * 28 + 15];
					
					cells[addedcell].Velocity = v;
					cells[addedcell].AngularVelocity = va;
					cells[addedcell].Ancestor = addedcell + 1;
					asdfgh = (float)rand() / RAND_MAX;
					cells[addedcell].Type = new_cell_array[icell * 28 + 2];

					if(cells[addedcell].Type==2){
						if (asdfgh < (0.5))
						{
							cells[addedcell].Type = 1;
						}
						else
						{
							cells[addedcell].Type = 2;
						}
					}

					cells[addedcell].GrowthRate = 0.0;

					//cells[addedcell].GrowthRate = 0.0;
					Grid.Add(addedcell, Grid.GetAddress(average(cells[addedcell].Position)));

					int icheck = 0;
					while (icheck < addedcell)
					{
						//					double dist;
						//					dist = sqrt((cm.x-(cells[icheck].Position.p.x+
						//						cells[icheck].Position.q.x)*0.5)*(cm.x-
						...
						//						+(cm.y-(cells[icheck].Position.p.y+cells[icheck].Position.q.y)*0.5)
						//						*(cm.y-(cells[icheck].Position.p.y+cells[icheck].Position.q.y)*0.5));
						min_distance(cells[addedcell], cells[icheck], dist, c1, c2);
						if (dist < (cells[addedcell].Radius + cells[icheck].Radius))
						{
							CheckOverlap = true;
							printf("Cells overlap!\n");
							RegenCellCount++;
					...
					if (icheck == addedcell) { CheckOverlap = false; }
					if (RegenCellCount == RegenCellMax)
					{
						printf("Unable to generate initial cells!\n");
						exit(0);
